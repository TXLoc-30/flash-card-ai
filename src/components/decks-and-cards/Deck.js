...
const Deck = ({ 
  ...
  useEffect(() => {
    console.log("Deck component useEffect triggered", { 
      hash, 
      shuffledCardsLength: shuffledCards.length, 
      hashCardsLength: hashCards?.length || 0 
    });
    
    setIsLoaded(false);
    let _cards = [];

    // If we have shuffledCards (from shuffle button), use them
    // Otherwise, if we have hash (from URL), use hashCards
    if (shuffledCards.length > 0) {
      _cards = shuffledCards;
      console.log("Using shuffledCards:", _cards.length);
    } else if (hashCards != null) {
      _cards = hashCards;
      console.log("Using hashCards:", _cards.length);
    }
    ...
    if (_cards.length > 0) {
      ...
      setIsLoaded(true);
      console.log("Cards set, isLoaded:", true);
    } else {
      console.log("No cards to display");
      setIsLoaded(true); // Still set loaded to true so we don't show spinner forever
    }

    }, [shuffledCards, isCardFlipped, onClick, hashCards, hash]
  );
